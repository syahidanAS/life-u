...
class UserController extends Controller
{
    ...
    public function update(Request $request)
    {
        try {
            ...
            $query->save();
            $query->syncRoles(Auth::user()->getRoleNames()->first() == 'Customer' ? 'Customer' : $request->role);
            $query->assignRole(Auth::user()->getRoleNames()->first() == 'Customer' ? 'Customer' : $request->role);
            return response()->json([
                'status' => 'success',
    ...
    {
        $id = Main::hashIdsDecode($id);
        $data = User::where('id', $id)->first();
        return view('customer-dashboard.profile', compact('data'));
    }
}
